<script lang="ts">
  ...
  // Draggable box
  let boxX = 670, boxY = 60, boxW = 220, boxH = 280;
  ...
  function onDrag(e: MouseEvent) {
    ...
    if (dragging === "move") {
      boxX = Math.max(0, Math.min(W - boxW, dragStart.bx + dx));
      boxY = Math.max(0, Math.min(H - boxH, dragStart.by + dy));
    } else {
  ...
  $: insideStates = centroids
    .filter(c => { const cx = c.x + oX, cy = c.y + oY; return cx >= boxX && cx <= boxX + boxW && cy >= boxY && cy <= boxY + boxH; })
    .sort((a, b) => (mortMap.get(a.code) ?? 0) - (mortMap.get(b.code) ?? 0));
